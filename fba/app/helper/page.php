...
class Page{
	...
	*/
	function navigation($parm = array()){

		$output = '<DIV class="navigation" style="float:left;width:400px;"><UL>';
		/*显示总页数*/
		...
		{
			$parm['page'] = $this->_pagination['first'];
			$url = QContext::instance()->url($this->_udi,$parm);
			$output = $output.'<LI><a href="javascript:loadTab(\''.$url.'\')">第一页</a></LI>';
		}
		else
		{
		...
		if($this->_pagination['current'] > 1 && $this->_pagination['last'] > 1 )
		{
			$parm['page'] = $this->_pagination['prev'];
			$url = QContext::instance()->url($this->_udi,$parm);
			$output = $output.'<LI><a href="javascript:loadTab(\''.$url.'\')">上一页</a></LI>';
		}
		else
		{
		...
		{
			$parm['page'] = $this->_pagination['next'];
			$url = QContext::instance()->url($this->_udi,$parm);
			$output = $output.'<LI><a href="javascript:loadTab(\''.$url.'\')">下一页</a></LI>';
		}
		else
		{
		...
		if($this->_pagination['current'] < $this->_pagination['last']  )
		{
			$parm['page'] = $this->_pagination['last'];
			$url = QContext::instance()->url($this->_udi,$parm);
			$output = $output.'<LI><a href="javascript:loadTab(\''.$url.'\')">最后一页</a></LI>';
		}
		else
		{
			$output = $output.'<LI>最后一页</LI></UL></DIV>';
		}
