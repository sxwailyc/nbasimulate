...
from webauth.common import log_execption, init_log
from webauth.common.constants import \
    ClientStatus, STATUS_MAP, Command, SmartClientCommand
...
#            self.work_thread.kill()
        ...
        while True:
            try: 
                ...
                if data['error'] is not None:
                    logging.warning('STATUS:%s, REPORT Server ERROR: %r' % (status, data['error']))
                else:
                    break
    ...
    def _register(self):
        ...
        while True:
            try:
                data = self.service.register(self._id, self.client_type, webauth.VERSION)
                if data['error'] is not None:
                    logging.error('%r' % data['error'])
                    return False
                self.client_id = data['result']
