...
window.onload = function() {
	...
	web_service_privilege = new rpc.ServiceProxy(
		"/services/accounts/privilege/",{
			asynchronous: false,
			methods:['get_user_privilege']
		});
};

var get_tasks_service = function(){
	try{
		var result = web_service.get_tasks_ex();
		return result;
	}catch(e){
		return null;
	}
}
var get_clients_ip_service = function(){
	try{
		var result = web_service.get_clients_ip();
		if(result !== undefined){
			return result;
		}
	}catch(e){}
...
var get_task_service = function(task_id){
	try{
		var result = web_service.get_task_ex(task_id);
		return result;
	}catch(e){
		return null;
	}
...
var save_task_service = function(task){
	...
	var reason = "";
	try{
		var result = web_service.add_task_ex(task); //[result, {'id': taskid, 'message': msg}]
		execute_ok = result[0];
...
var update_task_service = function(task){
	...
	var reason = "";
	try{
		var result = web_service.update_task_ex(task); //[result, {'id': taskid, 'message': msg}]
		execute_ok = result[0]; 
		reason = result[1]["message"];
	}catch(e){}
	if(!execute_ok){
...
var delete_task_service = function(task_id){
	...
	var reason = "";
	try{
		var result = web_service.delete_task_ex(task_id);//[result, {'id': taskid, 'message': msg}]
		execute_ok = result[0]; 
		reason = result[1]["message"];
	}catch(e){}
	if(!execute_ok){
...
var get_form_values = function(){
	...
	values['client_ip'] = $('#client_ip').val();
	return values;
}
...
var reset_form_values = function(){
	...
	Ext.get('client_ip').dom.selectedIndex = 0;
	$('#taskDesc').html("");
}
...
var format_form_values = function(task_info) {
	...
	$('#client_ip').val(task_info["client_ip"]);
	refresh_descript();}
   ...
   // Ext.QuickTips.init();
	...
	var task_operator_panel = new Ext.FormPanel({
	    ...
	    items: [tipfs, {
			...
			<tr>\
				<td style="text-align:center" width="132">部署机IP(<span class="fcRed">*</span>)</td>\
				<td style="text-align:left"><select style="width:60%" name="client_ip" id="client_ip" width="60px">\
			</td>\
			  </tr>\
			</table>\
			'
	    ...
	    buttons: [{
	        ...
	        handler: function(btn){
	        	...
	        	if($('#client_ip').val() == null){
	        		Ext.MessageBox.alert("错误","您至少需要选择一个IP地址！")
		        	return;
	        	}
	...
	var task_operator_window = new Ext.Window({
	    ...
	    listeners: {
	        "show": function(){
	        	$("#client_ip").empty();
	        	var ips = get_clients_ip_service();
            	if(ips !== undefined && ips != null){
            		for(var i=0;i<ips.length;i++){
            			var opt = document.createElement('option');
                    	opt.value = ips[i];
                    	opt.innerHTML = ips[i];
                    	$("#client_ip").append(opt);
            		}
            	}
            ...
            handler: function(){
                ...
                if(task_id != undefined){
                	Ext.MessageBox.confirm("确认","您确认要删除编号为[" + task_id+"]的任务吗？",
                		function(btn, text){
    						if (btn == 'yes'){
    							if(record_.get("taskname") == "sync_plantask"){
    								Ext.MessageBox.confirm("确认","您确认需要删除["+record_.get("ip")+"]的[同步计划任务]吗？",function(btn,text){
    									if (btn == 'yes'){
    ...
    grid_plan_task.getSelectionModel().on('selectionchange', function(sm){
        grid_plan_task.removeBtn.setDisabled(sm.getCount() < 1 || current_page_privilege < 2);
		grid_plan_task.modifyBtn.setDisabled(sm.getCount() < 1 || current_page_privilege < 2);
    });
    
    var refresh_privilege_enable = function(privilege){
    	//这里处理按钮是否是disable的
    	if(privilege < 2){
    		grid_plan_task.addBtn.setDisabled(true);
    		grid_plan_task.removeBtn.setDisabled(true);
    		grid_plan_task.modifyBtn.setDisabled(true);
    	}
    }
    		...
    		if(task_info["tasks"] !== undefined){
    			for(var i=0;i<task_info["tasks"].length;i++){
    				var record_ = new PlanTask({
    					...
    					ip: task_info["tasks"][i]["client_ip"],
    ...
    get_privilege_info(500, this, null);//打开页面先延时500毫秒查询权限
    refresh_task.defer(500, this, null);//再延时500毫秒显示一下任务
});
