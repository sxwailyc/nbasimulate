...
class Context(object):
    ...
    def __getattr__(self, name, force=True):
        if hasattr(self, name):
            return getattr(self, name)
        val = self._data.get(name)
        if force and not val:
            raise 'object %s not has attribute %s' % (self.__class__.__name__, name)
        return val
...
#    private int seq;
#
#    private int currentOffensiveCostTime = 0; // ﾵﾱￇﾰﾽ￸ﾹﾥￋ￹ￊﾹￓￃ￁ￋﾵￄￊﾱﾼ￤,ￒￔﾺ￁ￃ￫ￋ￣
#
#    private int currentTeam = MatchConstant.CURRENT_TEAM_A; // 0ﾴ￺ﾱ￭ￖ￷ﾶￓ﾿￘ￇ￲
#
#    private int previousActionAffect = 0;// ￉ￏￒﾻﾶﾯￗ￷ﾶￔﾱﾾﾴￎﾶﾯￗ￷ﾳ￉ﾹﾦￓ￫ﾷ￱ﾵￄￓﾰￏ￬
#
#    private int tacticsAffect = 0; // ￕﾽￊ￵ﾶￔﾶﾯￗ￷ﾳ￉ﾹﾦﾵￄￓﾰￏ￬
#
