...
from gba.common import logger, teamutil, exception_mgr
from gba.common.bottle.persistable import PersistableException
from gba.entity import League, LeagueMatchs, Matchs, LeagueTeams, SeasonFinance, \
                       TeamArena, Team, AllFinance, TeamTicketHistory, RoundUpdateLog
...
class DailyLeagueUpdate(BaseBetchClient):
    ...
    def _run(self):
        ...
        while True:
            ...
            for league in leagues:
                ...
                self._handle_matchs(league_matchs)
    ...
    def _save_round_log(self):
        '''保存日志'''
        round_update_log = RoundUpdateLog()
        round_update_log.season = self._season
        round_update_log.round = self._round
        round_update_log.match_count = self._total_created_match
        while True:
            try:
                round_update_log.persist()
                break
            except:
                exception_mgr.on_except()
                time.sleep(10)
                         
    def _handle_matchs(self, league_matchs):
        while True:
            Matchs.transaction()
            try:
                for league_match in league_matchs:
                    ...
                    if home_league_team.team_id != -1:
                        ...
                        if has_match:
                            ...
                            old_funds = home_team.funds
                            new_funds = float(old_funds) + float(str(amount))
                            home_team.funds = new_funds
