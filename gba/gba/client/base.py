...
import dtspider
from dtspider.client.rpc_proxy import client_service_proxy
from dtspider.common import init_log
from dtspider.common.constants import ClientStatus, STATUS_MAP, Command, SmartClientCommand
...
#                self._report() # 不由工作线程调用
    ...
    def _svnup(self):
        logging.warning("%s[Source Version = %s] SVNUP, exit 43" % (self.client_type, dtspider.VERSION))
...
#            self.work_thread.kill()
    ...
    def _register(self):
        ...
        while True:
            try:
                self.client_id = self.service.register(self._id, self.client_type, dtspider.VERSION)
