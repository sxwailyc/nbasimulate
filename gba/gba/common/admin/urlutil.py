...
URL_REASON = {
    ...
    0x04: '缓冲区攻击',
    0x05: '检测到其返回地址是在栈空间',
    0x06: '根据返回地址的页属性不符合要求',
    0x07: '命令行解析',
    0x08: '根据返回地址的页属性不符合要求',
    0x09: '恶意网址',
    0x0A: '恶意网址',
    0x0B: '恶意网址',
    0x0C: '恶意网址',
...
NOT_EY_REASONS = (0x01, 0x02, 0x05, 0x06, 0x08, 0x13, 0x15,)
FISH_REASONS = (0x10, 0x16, 0x18,)
EY_REASONS = (0x09, 0x0A, 0x0B, 0x0C,)

def _get_detail_by_reason_type(reason_type):
    if reason_type in FISH_REASONS or reason_type in EY_REASONS or reason_type == 0x1A:
        return None
    detail = URL_REASON.get(reason_type, None)
    if detail is None:
        detail = '未知原因(%s)攻击' % reason_type
    return detail

RE_REASON_DETAIL = re.compile(r'含有(.+?)恶意|试图触发(.+?漏洞)|试图(.+?签名)', re.IGNORECASE)
def get_reason_detail(reason):
    """获取友好的原因"""
    ...
    for r in reason:
        url = r[1]
        reason_type = int(r[0])
        if reason_type in NOT_EY_REASONS:
            continue
        if len(r) >= 3:
            desc = r[2]
        else:
            desc = u''
        if url:
            url_split = standardize(url)
            if url_split is not None:
                if reason_type in FISH_REASONS or (reason_type == 0x1A and u'欺诈' in desc):
                    fish_hosts.add(url_split.host)
                else:
                    ey_hosts.add(url_split.host)
        if len(r) >= 3:
            d = r[2]
            if isinstance(d, unicode):
                d = d.encode('utf-8')
            m = RE_REASON_DETAIL.search(d)
            if m:
                for detail in m.groups():
                    if detail:
                        detail = detail.lower()
                        details[detail] = details.get(detail, 0) + 1
            else:
                detail = _get_detail_by_reason_type(reason_type)
            ...
            if detail is not None:
                details[detail] = details.get(detail, 0) + 1
    return details, fish_hosts, ey_hosts

def format_reason(details, ey_hosts, fish_hosts):
    """格式化输出原因"""
    infos = []
    if details:
        infos.append('存在%s' % ', '.join(details.keys()))
    if fish_hosts:
        infos.append('访问欺诈网站%s' % ', '.join(fish_hosts))
    if ey_hosts:
        if not infos:
            infos.append('存在恶意代码攻击')
        infos.append('恶意攻击托管在%s' % ', '.join(ey_hosts))
    return ', '.join(infos)

def get_display_reason(reason):
    """获取用于显示的友好原因"""
    if isinstance(reason, basestring):
        reason = json.loads(reason)
    if not reason:
        return repr(reason)
    if len(reason[0]) >= 3: # 判断是否已格式化
        description = reason[0][2]
        if isinstance(description, unicode):
            description = description.encode('utf-8')
        if description.startswith('访问欺诈网站') or description.startswith('存在'):
            return description
    details, fish_hosts, ey_hosts = get_reason_detail(reason)
    display = format_reason(details, ey_hosts, fish_hosts)
    if not display:
        display = repr(reason)
    return display

def get_raw_display_reason(reason):
    """显示最原始的原因"""
    if isinstance(reason, basestring):
        reason = json.loads(reason)
    if not reason:
        return ''
    return (u'\n'.join([u'%s, %s, %s' % (r[0], r[2], r[1]) for r in reason if len(r) >= 3])).encode('utf-8')

KWS_TOP_URL_FLAG = 0x80000000
KWS_USER_DEFINE_FLAG = 0x40000000
KWS_URL_CLASS_FLAG = 0x3C000000
KWS_BLOCK_REASON_MASK = 0x000000FF
#0011 1100 0000 0000 0000 0000 0000 0000
def translate_webshield_reason(raw_reason):
    """解析网盾原始原因
    详细定义: http://trac.rdev.kingsoft.net/mercury/wiki/WebshieldReason
    """
    if isinstance(raw_reason, basestring):
        raw_reason = int(raw_reason, 16)
    top = (raw_reason & KWS_TOP_URL_FLAG) != 0
    user_define = (raw_reason & KWS_USER_DEFINE_FLAG) != 0
    url_class = (raw_reason & KWS_URL_CLASS_FLAG) >> 26
    reason = raw_reason & KWS_BLOCK_REASON_MASK
    return top, user_define, url_class, reason
...
#            hostname = "%s:%s" % (hostname, p)
    ...
    @property
    def home_page(self):
        hostname = self.host # 保证是utf8编码的str
        if not hostname:
            return None
        try:
            p = self.port
...
def _is_bad_url(url_split):
    """判断是否是坏url"""
    try:
        domain = _host_to_domain(url_split.host)
        if domain:
            return False
        else:
            return True
