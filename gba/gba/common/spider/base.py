...
#        url = self.urlencode(url)
        ...
        try:
            ...
            if 'next_level' in template:
                ...
                if hasattr(self, 'get_next_level_urls'):
                    next_level_urls = self.get_next_level_urls(content, template['next_level'], callback, level)
                else:
    ...
    def request(self, url, referer=None):
        ...
        try:
            ...
            if hasattr(e, 'reason'):
                #print 'sWe failed to reach a server.'
            ...
            urllib2.socket.setdefaulttimeout(old_timeout)
    ...
    def get_content(self, response):
        ...
        charset = self.CHARSET_RE.search(response.headers.get('content-type', '')) # 自动检测charset，若有，则decode
        if not charset:
