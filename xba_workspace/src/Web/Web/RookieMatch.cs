namespace Web
{
    ...
    public class RookieMatch : Page
    {
        ...
        {
            int intRookieOpIndex = AccountItem.SetRookieOp(this.intUserID, 6);
            if (intRookieOpIndex != 6)
            {
                base.Response.Write("<script>window.top.Main.location=\"" + StringItem.GetRookieURL(intRookieOpIndex).ToString().Trim() + "\";</script>");
            }
            else
            {
                base.Response.Write("<script>window.top.Main.location=\"" + StringItem.GetRookieURL(7).ToString().Trim() + "\";</script>");
            }
        }
