    ...
    public class ChooseClub : Page
    {
        ...
        public void GetList()
        {
            foreach (DataRow row in BTPAccountManager.GetChooseClub(this.intUserID).Rows)
            {
                ...
                string str3 = row["ClubLogo"].ToString().Trim();
                string dev = DevCalculator.GetDev(row["DevCode"].ToString().Trim());
                this.sbList.Append("<tr class='BarContent' onmouseover=\"this.style.backgroundColor='#FBE2D4'\" onmouseout=\"this.style.backgroundColor=''\"><td width=60 align=center valign=middle ><img src='" + str3 + "' width=46 height=46></td>");
                ...
                this.sbList.Append("<td width=70 align=center><a href='Temp_Right.aspx?Type=STREENLEV' target=Right>" + num + "</a></td>");
